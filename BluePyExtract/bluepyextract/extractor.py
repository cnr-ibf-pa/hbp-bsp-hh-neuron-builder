...
class Extractor(object):
    ...
    def process_file(self, filename, cellname, expname, i_file):
		...
		#logger.info(typeof(filename))
            ...
            for i_seg, seg in enumerate(bl.segments):
                ...
                # llb voltage[ion:ion+int(numpy.ceil(0.4/dt))] = voltage[ion+int(numpy.ceil(0.4/dt))]
                # llb voltage[ioff:ioff+int(numpy.ceil(0.4/dt))] = voltage[ioff+int(numpy.ceil(0.4/dt))]
		...
		#logger.info(self.cells[cellname])
		#logger.info(amp)
        ...
        for i_cell, cellname in enumerate(self.dataset):
            ...
            for i_exp, expname in enumerate(dataset_cell_exp):
                ...
                for i_seg in range(len(voltages)):
                    ...
                    for feature in features_all:
                        #logger.info("computing feature: %s", feature)
    ...
    def get_threshold(self, amp, numspikes):
        ...
        #i_threshold = numpy.where(numspikes_sort>=1)[0][0] # commented by llb
        #amp_threshold = amps_sort[i_threshold] # commented by llb
        amp_threshold = amps_sort[0]
    ...
    def feature_config_cells(self):
        for i_cell, cellname in enumerate(self.dataset):
            dataset_cell_exp = self.dataset[cellname]['experiments']
            self.create_feature_config(self.maindirname+cellname+os.sep, dataset_cell_exp)

...
class CheckConditions:
    ...
    @classmethod
    def check_uploaded_model(cls, file_path="", folder_path=""):
        """
        Check that the uploaded model .zip file contains all necessary files and folder
        """
        ...
        if not os.path.exists(opt_folder):
            return {"response": "KO", "message": "The unzipped folder has not the same name as the .zip file.<br>Please upload a well formatted .zip"}
        else:
            ...
            for f in folder_list:
                if not os.path.exists(os.path.join(opt_folder, f)):
                    return {"response": "KO", "message": "Folder/file '" + f + "' does not exist \
                            in the optimization folder.<br>Please upload a well formatted .zip file"}
            ...
            for i, j in enumerate(jsonfile_list):
                ...
                try:
                    ...
                    return {"response": "KO", "message": "File '" + c_filename + "' is either not \
                            present or not readable in 'config' folder.<br>Please check your .zip file"}
            
            all_keys = list(set(keys))
            if len(all_keys) != 1:
                return {"response": "KO", "message": "All .json files in the 'config' folder must \
                        contain the same data key.<br>Please upload a well formatted .zip file"}
            else:
                for line in open(os.path.join(opt_folder, "opt_neuron.py")):
                    if line.startswith('evaluator = model.evaluator.create'):
                        ...
                        if opt_key != all_keys[0]:
                            return {"response": "KO", "message": "Line 75 in 'opt_neuron.py' must contain the same key as the one \
                                    contained in the .json files in the 'config' folder.<br>Please upload a well formatted .zip file"}
            ...
            if not file_list or len(file_list) != len(modfilelist):
                return {"response": "KO", "message": "The folder 'mechanisms' must not be empty and must contain only .mod files"}
            ...
            for f in file_list:
                if not os.path.exists(os.path.join(opt_folder, "model", f)):
                    return {"response": "KO", "message": "File '" + f + "' does not exist in the \
                            'model' folder.<br>Please upload a well formatted .zip file"}
            ...
            for f in file_list:
                if not os.path.exists(os.path.join(opt_folder, "tools", f)):
                    return {"response": "KO", "message": "File '" + f + "' does not exist in the \
                            'tools' folder.<br>Please upload a well formatted .zip file"}
            ...
            if len(morph_list_dir) != 1:
                return {"response": "KO", "message": "Folder 'morphology' must contain only one file. " + str(len(morph_list_dir)) +
                        "files are present in the folder.<br>Please upload a well formatted .zip file"}
            else:
            ...
            if not modlist:
                return {"response": "KO", "message": "No .mod file in folder 'mechanisms'.<br>Please check your .zip file"}

            return {"response": "OK", "message": "Folder exists"}

    @classmethod
    def check_sim_folders(cls, folder_path=""):

        if not os.path.isdir(folder_path):
            return {"response": "KO", "message": "Optimization result does not exist. Check your files"}
...
class FetchFiles:
    ...
    @classmethod
    def fetch_opt_set_file(cls, opt_set_file_path=""):

...
function TracePlot(container_id, cell_obj) {
    var n_traces = cell_obj['traces'].length

    this.container = $('#' + container_id)
    this.cell_obj = cell_obj
    
    this.appearance = new Array(n_traces).fill(SHOW_FADED)
        
    function init() {
        var md5 = cell_obj['md5']
        var timestamp = Date.now()
        
        this.cellinfo = [this.cell_obj['species'], this.cell_obj['area'],
                         this.cell_obj['region'], this.cell_obj['type'],
                         this.cell_obj['etype'], this.cell_obj['name'],
                         this.cell_obj['sample']]
        ...
        this.infobox.append('<span></span>').text('Cell properties: ' + this.cellinfo.join(' -> '))
        this.infobox.append('<br><br>')
        this.infobox.append('<button class="selall btn btn-link btn-default">Select all</button>')
        this.infobox.append('<button class="dselall btn btn-link btn-default">Deselect all</button>')
        this.infobox.append('<button class="invsel btn btn-default btn-link">Invert selection</button>')
    }

    function plot() {
        var that = this
        var plotdata = []
        $.each(this.cell_obj['traces'], function(key, trace) {
            that.formbox.append('<input type="checkbox" name="' + key + '" />')

            // Defines what is about to be plotted
            var newTrace = {
                y: trace,
                name: key + ' ' + that.cell_obj['amp_unit'],
                mode: 'lines',
                hoverinfo: 'none',
                opacity: SHOW_FADED
            }
        ...
        //this.opacities = new Array(n_traces).fill(SHOW_FADED)
        //var opacity_before_hover = null

        Plotly.newPlot(this.plotbox.attr('id'), plotdata, layout, {displayModeBar: false})//.then(manageLegend)
        this.refresh()
    ...
    this.refresh = function() {
        ...
        Plotly.restyle(this.plotbox.attr('id'), update)//.then(manageLegend)

        // Sets the opacities of the legend's labels
        legend = Plotly.d3.select('#' + this.plotbox.attr('id') + ' g.legend')
...
TracePlot.prototype.init = function() {
    console.log('init')
}

...
def disable_cypress_video(project):
    conf = 'cypress.json'
    if conf in os.listdir(project):
        with open(os.path.join(project, conf), 'r') as fd:
            jj = json.load(fd)
        jj.update({'video': False})
        with open(os.path.join(project, conf), 'w') as fd:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--host', type=str, default='http://127.0.0.1:8000', help='Set the host to use. Default is http://127.0.0.1:8000.')
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=args.max_processes) as executor:
        processes = [executor.submit(run_cypress, args.cypress, args.project, args.host, args.browser, args.enable_browser) for _ in range(args.test_number)]
